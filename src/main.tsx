...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="cockpit">
      {!selected.readOnly && <RecordingHero state={recorderState} url={recordUrl} urlError={recordUrlError} message={recorderMessage} stepCount={recordedSteps.length} available={Boolean(window.studio)} canRecord={canRecord} onUrlChange={setRecordUrl} onStart={() => void startRecording()} onStop={() => void stopRecording()} />}
...
function RecordingHero({ state, url, urlError, message, stepCount, available, canRecord, onUrlChange, onStart, onStop }: { state: RecorderState; url: string; urlError: string; message: string; stepCount: number; available: boolean; canRecord: boolean; onUrlChange: (value: string) => void; onStart: () => void; onStop: () => void }) {
  return <section className={`recording-hero recording-hero-${state}`} aria-labelledby="recording-hero-title"><div className="recording-hero-copy"><div className="eyebrow">BROWSER RECORDER</div><h1 id="recording-hero-title">Turn browser actions into tests.</h1><p>Start from a URL, interact with the page, and keep every useful step.</p><small className={!available ? 'browser-mode-message' : undefined}>{!available ? 'Recorder requires Playwright Studio Desktop.' : state === 'recording' ? `${stepCount} steps captured` : state === 'stopping' ? 'Finishing recording…' : 'Record actions from a controlled Chromium window'}</small><div className="recording-hero-form">{state === 'recording' || state === 'stopping' ? <button className="stop-record" onClick={onStop} disabled={state === 'stopping'}>■ Stop recording</button> : <div className="record-start"><div><input value={url} onChange={(event) => onUrlChange(event.target.value)} placeholder="https://example.com" aria-label="Starting URL" aria-invalid={Boolean(url && urlError)} aria-describedby="record-url-help" /><small id="record-url-help">{urlError || 'Enter the starting URL for Chromium.'}</small></div><button className="record" onClick={onStart} disabled={!canRecord}>● Record</button></div>}</div></div><div className="recording-hero-status" aria-live="polite"><span className="recording-status-orbit" aria-hidden="true" /><strong>{state === 'recording' ? 'Recording browser actions' : state === 'stopping' ? 'Finishing capture' : state === 'error' ? 'Recorder error' : 'Ready to record'}</strong><span>{state === 'recording' ? `${stepCount} steps captured` : state === 'error' ? message || 'The recorder encountered an error. Try again.' : 'Chromium session will open when you start.'}</span></div></section>;
}
