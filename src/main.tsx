  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      <section className="builder"><div className="section-head"><div><div className="eyebrow">TEST BUILDER</div><input className="test-title" value={selected.name} readOnly={selected.readOnly} onChange={(event) => update({ name: event.target.value })} /></div><div className="builder-actions">{!selected.readOnly && <><button className="ghost" onClick={undo} disabled={!historyRef.current.canUndo}>Undo</button><button className="ghost" onClick={redo} disabled={!historyRef.current.canRedo}>Redo</button><button className="ghost" onClick={renameSelected} disabled={!state || !window.studio}>Rename</button><button className="ghost danger-action" onClick={deleteSelected} disabled={!state || !window.studio}>Delete</button><button className="ghost" onClick={() => update({ steps: [] })}>Clear</button><button className="primary" onClick={save} disabled={!canSave}>Save test</button></>}</div></div>
        {!selected.readOnly && <section className={`recording-hero recording-hero-${recorderState}`} aria-labelledby="recording-hero-title"><div className="recording-hero-copy"><div className="eyebrow">BROWSER RECORDER</div><h1 id="recording-hero-title">Turn browser actions into tests.</h1><p>Start from a URL, interact with the page, and keep every useful step.</p><small className={!window.studio ? 'browser-mode-message' : undefined}>{!window.studio ? 'Recorder requires Playwright Studio Desktop.' : recorderState === 'recording' ? `${recordedSteps.length} steps captured` : recorderState === 'stopping' ? 'Finishing recording…' : 'Record actions from a controlled Chromium window'}</small><div className="recording-hero-form">{recorderState === 'recording' || recorderState === 'stopping' ? <button className="stop-record" onClick={stopRecording} disabled={recorderState === 'stopping'}>■ Stop recording</button> : <div className="record-start"><div><input value={recordUrl} onChange={(event) => setRecordUrl(event.target.value)} placeholder="https://example.com" aria-label="Starting URL" aria-invalid={Boolean(recordUrl && recordUrlError)} aria-describedby="record-url-help" /><small id="record-url-help">{recordUrlError || 'Enter the starting URL for Chromium.'}</small></div><button className="record" onClick={startRecording} disabled={!canRecord}>● Record</button></div>}</div></div><div className="recording-hero-status" aria-live="polite"><span className="recording-status-orbit" aria-hidden="true" /><strong>{recorderState === 'recording' ? 'Recording browser actions' : recorderState === 'stopping' ? 'Finishing capture' : recorderState === 'error' ? 'Recorder error' : 'Ready to record'}</strong><span>{recorderState === 'recording' ? `${recordedSteps.length} steps captured` : recorderState === 'error' ? recorderMessage || 'The recorder encountered an error. Try again.' : 'Chromium session will open when you start.'}</span></div></section>}
