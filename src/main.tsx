...
const stepError = (step: Step) => {
  ...
  if (step.type !== 'navigate' && step.type !== 'wait' && step.type !== 'screenshot' && !step.selector?.trim()) return 'Locator is required';
  if (['fill', 'select', 'upload', 'press'].includes(step.type) && !step.value?.trim()) return 'Value is required';
  if (step.type === 'assert' && ['text', 'value', 'url', 'urlContains', 'attribute', 'count', 'title'].includes(step.assertion || '') && !step.value?.trim()) return 'Expected value is required';
