...
function RunPanel({ status, result, message, available, headed, onHeadedChange, onRun, onStop }: { status: RunStatus; result: RunResult | null; message: string; available: boolean; headed: boolean; onHeadedChange: (value: boolean) => void; onRun: () => void; onStop: () => void }) {
  ...
  const reportStatus = report?.failed ? 'failed' : result?.status;
  const failureLocation = report?.errorLocation && `${report.errorLocation.file}${report.errorLocation.line === undefined ? '' : `:${report.errorLocation.line}`}${report.errorLocation.column === undefined ? '' : `:${report.errorLocation.column}`}`;
